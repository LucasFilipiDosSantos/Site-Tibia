...
internal sealed class InMemoryRefreshSessionRepository : IRefreshSessionRepository
{
    ...
    {
        return Task.FromResult(Sessions.SingleOrDefault(s => s.TokenHash == tokenHash && !s.IsRevoked && !s.IsExpired(nowUtc)));
    }
...
internal sealed class InMemorySecurityTokenRepository : ISecurityTokenRepository
{
    ...
    {
        return Task.FromResult(Tokens.SingleOrDefault(t => t.TokenHash == tokenHash && t.Purpose == purpose && !t.IsConsumed && !t.IsExpired(nowUtc)));
    }
